...
interface ServicePageProps {
  ...
  photosTitle: string;
  breadcrumb: string;
  ctaLabel?: string; // Текст кнопки (необязательно). Если не задан — кнопки нет
  ctaHref?: string; // Ссылка кнопки (по умолчанию #catalog)
}
      ...
      <section className="py-16 md:py-20 bg-slate-50 dark:bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-10">
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white">{photosTitle}</h2>
            <p className="text-slate-600 dark:text-slate-400 mt-3">Галерея выполненных работ — скоро появится</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              ...
              >
                <div className="text-5xl mb-2">{photosIcon}</div>
