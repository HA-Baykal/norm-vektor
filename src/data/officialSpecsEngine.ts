...
const EXACT_OFFICIAL_PHOTOS_BY_ID: Record<number, string[]> = {
  ...
  702: [ // Kentatsu Атама Инвертор (Atama Inverter) (Инверторный)
    ...
    "https://daichi.business/upload/iblock/753/vrqfevfmw2vtm29or9widl3s7y4sn984/z3vsvpaf3yhgtiqdlu9t35sbhr888m1t.jpg",
    "https://daichi.business/upload/iblock/957/af7hby0cpgo777x6eyqk8j8vz6fxvbc6/6383taq2sqhv305o2u6s05m7wymjeb8y.jpg",
    "https://daichi.business/upload/iblock/9b0/fm8hnssjc1mc7p2ehnpp2ut0waownvgq/d4mbie20uanaloee3t7o8riy0ji9vj1v.jpg"
  ],
  703: [ // Kentatsu Харуки (Haruki) (Обычный)
    ...
    "https://daichi.business/upload/iblock/213/nzhkk6qyrg6eeubgmpwhcpa2nbl71qov/7c5r13ja8uoe5q84f6qvme8k2um8e4y2.jpg",
    "https://daichi.business/upload/iblock/bfd/l2fcrn5rozjhblybide8h2wkxy42jwmc/o0i65awor2oh085ezxnmdnm7wf8t292a.jpg"
  ],
  ...
  706: [ // Kentatsu Тиба (Tiba) (Обычный)
    ...
    "https://daichi.business/upload/iblock/982/qho63m2lm9uv4b26wpuvwruyqyjnsb4o/1zhjf20krhp5k9940mk5dimbi5fcbawt.jpg",
    "https://daichi.business/upload/iblock/943/qw1eaxbjntaya3sg6zb8itsbl1y5ikzh/yomyn4ru3f3k1lajdi2zgucrdfpt1xtb.jpg",
    "https://daichi.business/upload/iblock/9fd/jzhrbsocc3w279qwoqazzxwymvr9msgd/x0bzf317cge5o9os7muvp9777sfzzwtq.jpg",
    "https://daichi.business/upload/iblock/c29/4jk4bvalr462xl66va21xnziatw12un2/kvqd9oni1ybwn4usr2hhkelh89acftl6.jpg"
  ],
  ...
  708: [ // Kentatsu Отари Инвертор (Otari) (Инверторный)
    ...
    "https://daichi.business/upload/iblock/934/wtma1pjt8n4iearbuzqdr06ldbtunfxr/5b727c29443679d7b994ede0a4f0174b.jpg",
    "https://daichi.business/upload/iblock/ec4/ran96fk56f0qlzbc40f3k95rgbq0sj1l/fvs84zoacyoes4etcpj141evhzd4ucs9.jpg"
  ],
  709: [ // Kentatsu Семпай Инвертор (Sempai) (Инверторный)
    "https://daichi.business/upload/iblock/5f7/cq4etos1axlspiy21prliijcaknurk3i/pt627l1g4wvgg977c8zlb9z836ln48eu.jpg",
    "https://daichi.business/upload/iblock/f86/q8ps9ypccyjzvy2y51gtm0w7gf2sf81a/g9fkedp7hv0773vrmd32l4oik1n2t5pd.jpg",
    "https://daichi.business/upload/iblock/4ef/thfq4i58r5st8rn1c1w3sf22zmgs8uuw/cjd42icyovsgob3h7yrwa48zmw1vsp5b.jpg",
    "https://daichi.business/upload/iblock/63a/6ucizrrwqalyeft0r2vcwd9y93740iqb/ym5xleyerpnqih9frn58wfa9jg9w81q2.jpg"
  ],
  ...
  711: [ // Kentatsu Тамаши Инвертор (Tamashi) (Инверторный)
    ...
    "https://daichi.business/upload/iblock/a74/nag0g26u1md0tt1usrl3sfn138zd6yg2/3frkrif6bsetmkm7nex59l820bj64xhi.jpg",
    "https://daichi.business/upload/iblock/2ef/tjl9nl10bgevsolaak5icrtqqbe20gmq/5zb56o1ptfl3qq5tqe06cvnxtwepunek.jpg"
  ],
...
export function getOfficialSpecification(item: Conditioner, selectedBtu: number): OfficialSpecification {
  ...
  // Описание из официальных каталогов Русклимата и Даичи
  let fullDescription = "";
  if (isCassette) {
    fullDescription = `${item.name} — профессиональная полупромышленная сплит-система из официального каталога дистрибьютора ${distributor}. Предназначена для монтажа в подвесной потолок торговых, офисных или коммерческих помещений в Иркутске. Равномерный 360-градусный круговой поток воздуха исключает образование сквозняков и застойных зон. Встроенный дренажный насос поднимает конденсат на высоту до 750 мм.`;
  } else if (brandUpper === "SHUFT") {
    fullDescription = `Дизайн-серия сплит-систем ${item.name} от Русклимат отличается лаконичными формами, органично сочетающимися с любыми стилями интерьеров. Главным отличием стала максимальная забота о создании здорового микроклимата: приборы оснащены ионизатором, насыщающим воздух полезными анионами и устраняющим пыль, бактерии и запахи. Функция I-Feel отслеживает температуру по пульту ДУ, а 3D-поток предотвращает сквозняки.`;
  } else if (brandUpper === "BALLU") {
    fullDescription = `${item.brand} ${item.name} — бестселлер климатического каталога Русклимата, адаптированный для сибирского климата Иркутска. Теплообменники защищены специальным золотистым антикоррозийным покрытием Golden Fin, которое отталкивает влагу и в 3 раза повышает устойчивость к окружающей среде. Встроенный режим SLEEP и автоматическое качание жалюзи создают идеальную атмосферу для отдыха.`;
  } else if (brandUpper === "DAIKIN") {
    fullDescription = `Daikin ${item.name} — эталон японского инженерного искусства со склада официальной поставки Daichi. Модель оснащена запатентованным компрессором Daikin Swing с качающимся ротором, что снижает трение и вибрации к минимуму, гарантируя срок службы 15-20 лет. Система титано-апатитовой очистки воздуха разрушает молекулы запахов, а бесшумная работа (${minNoise}) делает эту модель №1 для спальни.`;
    ...
    fullDescription = `Сплит-система Electrolux ${item.name} сочетает скандинавский дизайн и технологии многоступенчатой очистки воздуха. Встроенная система защиты от скачков напряжения (160В–250В) оберегает прибор при нестабильной электросети в частном секторе. Функция I-Feel точно контролирует климат именно в той точке комнаты, где находится пульт управления.`;
  } else if (brandUpper === "KENTATSU" || brandUpper === "MIDEA" || brandUpper === "DAICHI" || brandUpper === "BOSCH" || brandUpper === "AXIOMA") {
    fullDescription = `Модель ${item.brand} ${item.name} из профессионального каталога Даичи (Daichi) комплектуется надежным японским компрессором ${compressorBrand} и высокоэффективным теплообменником с внутренним оребрением труб. Система автоматики включает самодиагностику, защиту от обдува холодным воздухом при обогреве (Anti-Cold Air) и функцию самоочистки испарителя при температуре +56°C.`;
  } else {
    fullDescription = `${item.name} — современное климатическое решение со склада официального дистрибьютора в Иркутске. Оснащается надежным компрессором ${compressorBrand}, режимом Turbo для быстрого выхода на нужную температуру и экономичным ночным режимом Sleep.`;
  }
