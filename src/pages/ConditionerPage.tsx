...
export default function ConditionerPage() {
  ...
  // Швейцарские часы: строгий поиск без fallback на первый товар — мягкий 404 запрещён
  const foundItem = conditioners.find((c) =>
    c.id.toString() === slug ||
  ...
  // Если товар не найден — честный 404 с noindex, редирект в каталог
  if (!foundItem) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="text-center">
          <h1 className="text-3xl font-black text-[#1a3a5c]">Модель не найдена</h1>
          <p className="mt-3 text-slate-600">Похоже, такой кондиционер снят с производства или ссылка устарела.</p>
          <Link to="/kondicionery" className="mt-6 inline-flex px-6 py-3 rounded-full bg-[#ff6b35] text-white font-black">← В каталог кондиционеров</Link>
        </div>
      </div>
    );
  }
  ...
  // SEO — швейцарские часы: Title никогда не пустой, canonical без ?btu, og:url чистый
  useEffect(() => {
    const safeFullName = fullName || item.name || "Кондиционер";
    ...
    updateMeta("og:image", coverPhoto.startsWith("http") ? coverPhoto : `${SITE_ORIGIN}${coverPhoto.startsWith("/") ? "" : "/"}${coverPhoto}`, true);
    updateMeta("og:url", cleanUrl, true);
    updateMeta("og:type", "product", true);
    setCanonical(cleanUrl);

    // Product schema с чистой ценой
    const productSchema = {
      "@context": "https://schema.org/",
      "@type": "Product",
      "name": `${safeFullName} (${selectedBtu} BTU)`,
      "image": allImages.map(img => img.startsWith("http") ? img : `${SITE_ORIGIN}${img.startsWith("/") ? "" : "/"}${img}`),
  ...
  }, [item, fullName, variant, selectedBtu, officialSpecs.minNoise, coverPhoto, allImages, slug, isMobile, isInstallOnRequest]);
  ...
  const handleMaxClick = () => window.open(MAX_LINK, "_blank", "noopener,noreferrer");
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(`🔗 Ссылка на модель ${item.name} скопирована!`);
      } catch {
        alert(`Ссылка: ${shareData.url}`);
      }
    }
  };
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-10 items-start">
            <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-24">
              <button type="button" onClick={() => setZoomOpen(true)} className="relative max-w-sm mx-auto w-full h-64 sm:h-72 rounded-3xl overflow-hidden bg-white border border-slate-200 shadow-lg p-4 flex items-center justify-center cursor-zoom-in">
                <div className="absolute left-3 top-3 z-10 flex flex-col gap-1.5">
                  {item.badge && <span className="rounded-full bg-[#ff6b35] px-3 py-1 text-[11px] font-black text-white shadow-sm w-fit">{item.badge}</span>}
              ...
              {allImages.length > 1 && (
                <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto">
                  {allImages.map((imgUrl, idx) => (
                    <button type="button" key={idx} onClick={() => setActiveImageIdx(idx)} className={`relative aspect-video rounded-xl overflow-hidden border-2 transition bg-white p-1 shadow-sm flex items-center justify-center ${activeImageIdx === idx ? "border-[#ff6b35] ring-2 ring-[#ff6b35]/25" : "border-slate-200 opacity-70 hover:opacity-100"}`}>
              ...
              <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-sm text-xs text-slate-600 space-y-2 max-w-sm mx-auto">
                ...
                <p className="leading-relaxed">
                  {isMobile ? "Мы работаем со складами официальных дистрибьюторов в Иркутске. На мобильный кондиционер предоставляется официальная гарантия завода." : "Мы работаем со складами официальных дистрибьюторов в Иркутске. На всю климатическую технику предоставляется гарантия завода, а наш монтаж сопровождает эту гарантию на весь период гарантии сплит-системы."}
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xl space-y-6">
                <div>
                  <div className="text-xs font-black uppercase tracking-wider text-[#ff6b35]">{item.type} кондиционер</div>
                  <h1 className="mt-1 text-2xl sm:text-3xl font-black text-[#1a3a5c] leading-tight">{item.name}</h1>
                  <p className="text-xs sm:text-sm font-bold text-slate-500 mt-2 flex flex-wrap items-center gap-2 sm:gap-3">
                    <span>🌍 Страна: {item.country}</span><span>•</span><span className="text-emerald-600 font-extrabold">🔇 Шум: {officialSpecs.minNoise}</span><span>•</span><span className="text-blue-600">⚡ {officialSpecs.energyClass}</span>
                  </p>
                </div>

                <div>
                  <div className="mb-2.5 text-xs font-black uppercase tracking-wider text-slate-500">Выберите мощность и площадь:</div>
                  <div className="flex flex-wrap gap-2.5">
                    {item.variants.map((v) => (
                      <button key={v.btu} type="button" onClick={() => handleSelectBtu(v.btu)} className={`rounded-2xl px-4 py-3 text-xs sm:text-sm font-black transition-all shadow-sm flex flex-col items-center gap-0.5 ${selectedBtu === v.btu ? "bg-[#1a3a5c] text-white ring-2 ring-[#ff6b35]" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                        <span className="text-base">{v.btu.toLocaleString("ru-RU")} BTU</span>
                        <span className={`text-[10px] font-semibold ${selectedBtu === v.btu ? "text-slate-300" : "text-slate-500"}`}>до {v.area} м² ({v.cooling})</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 p-4 rounded-2xl bg-slate-50 border border-slate-200/70 text-center text-xs">
                  <div><span className="text-slate-400 font-bold block text-[10px]">Площадь комнаты</span><strong className="text-[#1a3a5c] text-sm font-black">до {variant.area} м²</strong></div>
                  <div><span className="text-slate-400 font-bold block text-[10px]">Охлаждение</span><strong className="text-slate-800 text-sm font-black">{variant.cooling}</strong></div>
                  <div><span className="text-slate-400 font-bold block text-[10px]">Обогрев зимой</span><strong className="text-slate-800 text-sm font-black">{variant.heating}</strong></div>
                </div>

                {isMobile ? null : isInstallOnRequest ? (
                  <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4 text-sm font-bold text-slate-700">📐 {item.type === "Промышленный" ? "Монтаж промышленных моноблоков" : "Монтаж полупромышленных сплит-систем"} рассчитывается индивидуально после осмотра объекта в Иркутске</div>
                ) : (
                  <label className="flex cursor-pointer items-center justify-between gap-3 rounded-2xl bg-slate-50 border border-slate-200 p-4 hover:border-[#ff6b35] transition">
                    <div><span className="text-sm font-black text-[#1a3a5c] block">+ Стандартный монтаж под ключ</span><span className="text-xs font-extrabold text-emerald-600 block mt-0.5">Стоимость установки: {formatRub(INSTALL_PRICE)}</span></div>
                    <input type="checkbox" checked={withInstall} onChange={(e) => setWithInstall(e.target.checked)} className="h-6 w-6 shrink-0 accent-[#ff6b35] rounded cursor-pointer" />
                  </label>
                )}

                <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase">{isInstallOnRequest ? "Цена оборудования со склада" : isMobile ? "Цена кондиционера со склада" : withInstall ? "Кондиционер + установка" : "Цена кондиционера со склада"}</div>
                    <div className="flex items-end gap-3 mt-1">
                      <div className="text-3xl sm:text-4xl font-black text-[#1a3a5c]">{formatRub(totalPrice)}</div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <button type="button" onClick={() => setBookingOpen(true)} className="w-full sm:w-auto px-8 py-4 rounded-full bg-[#ff6b35] hover:bg-[#e95620] active:scale-95 text-white font-black text-base shadow-xl shadow-[#ff6b35]/30 transition text-center shrink-0">Заказать</button>
                    <button type="button" onClick={handleMaxClick} className="w-full sm:w-auto px-7 py-4 rounded-full bg-gradient-to-r from-blue-700 to-indigo-700 hover:from-blue-600 hover:to-indigo-600 active:scale-95 text-white font-black text-base shadow-lg transition text-center shrink-0 flex items-center justify-center gap-2"><span className="text-lg">🔥</span><span>MAX</span></button>
                    <button type="button" onClick={handleShare} title="Отправить ссылку" className="w-full sm:w-auto px-6 py-4 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-black text-sm active:scale-95 transition text-center shrink-0 flex items-center justify-center gap-2 border border-slate-200/80 shadow-sm"><span className="text-base">↗️</span><span>Поделиться</span></button>
                  </div>
                </div>

                <div className="pt-4 border-t border-slate-100">
                  <div className="rounded-2xl border-2 border-slate-200 overflow-hidden bg-slate-50 transition-all duration-200">
                    <button type="button" onClick={() => setSpecsOpen(!specsOpen)} className="w-full p-5 sm:p-6 text-left flex items-center justify-between gap-3 font-black text-base text-[#1a3a5c] hover:bg-slate-100 transition focus:outline-none">
                      <span className="flex items-center gap-2.5"><span className="text-[#ff6b35] font-extrabold text-lg">📋</span><span>Характеристики и техпаспорт (нажмите для раскрытия)</span></span>
                      <span className={`text-xs font-bold px-3 py-1 rounded-full bg-white border border-slate-300 text-slate-700 transition ${specsOpen ? "bg-[#ff6b35] text-white border-transparent" : ""}`}>{specsOpen ? "Свернуть ▲" : "Развернуть ▼"}</span>
                    </button>
                    {specsOpen && (
                      <div className="p-6 pt-4 border-t border-slate-200 bg-white space-y-6 text-sm animate-fade-in-up">
                        <div className="space-y-2"><div className="text-xs font-black uppercase text-[#ff6b35] tracking-wider">Официальная спецификация и обзор модели</div><p className="text-slate-700 leading-relaxed text-sm sm:text-base">{officialSpecs.fullDescription}</p></div>
                        <div>
                          <p className="font-black text-slate-800 mb-3 text-base">Технический паспорт кондиционера {item.name}:</p>
                        ...
                        <div>
                          <p className="font-black text-slate-800 mb-3 text-base">Особенности и встроенный функционал:</p>
                          <div className="grid sm:grid-cols-2 gap-2.5">
                            {officialSpecs.officialFeatures.map((f, i) => (
                              <div key={i} className="flex items-start gap-2 p-3 rounded-xl bg-slate-50 text-slate-800 text-xs font-bold border border-slate-100"><span className="text-[#ff6b35] font-black text-sm">✓</span><span className="leading-snug">{f}</span></div>
                            ))}
                          </div>
                        </div>
                        <div className="p-4 rounded-2xl bg-[#1a3a5c] text-white text-xs space-y-1"><div className="font-extrabold text-amber-400">💡 Сервис «Вектор Комфорта» в Иркутске:</div><div>{isMobile ? "Мы работаем со складами официальных дистрибьюторов в Иркутске. На мобильный кондиционер предоставляется официальная гарантия завода." : "Мы работаем со складами официальных дистрибьюторов в Иркутске. На всю климатическую технику предоставляется гарантия завода, а наш монтаж сопровождает эту гарантию на весь период гарантии сплит-системы."}</div></div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 text-xs text-slate-400 font-semibold">
                  <Link to="/kondicionery" className="hover:text-[#ff6b35] transition flex items-center gap-1"><span>← Вернуться ко всем кондиционерам в каталоге</span></Link>
                  <a href="tel:+79149146606" className="hover:text-[#1a3a5c] transition font-bold text-slate-600">Т.: +7 (914) 914-66-06</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-14 sm:py-20 bg-white border-t border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
            <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#ff6b35]">Рекомендации</p><h2 className="mt-2 text-2xl sm:text-3xl font-black text-[#1a3a5c]">Другие модели в наличии в Иркутске</h2></div>
            <Link to="/kondicionery" className="text-sm font-black text-[#ff6b35] hover:underline">Смотреть весь каталог →</Link>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            {conditioners.filter(c => c.id !== item.id).slice(0, 3).map((c) => {
              ...
              return (
                <Link key={c.id} to={`/kondicionery/${getModelUrlSlug(c)}`} className="group flex flex-col overflow-hidden rounded-[1.5rem] bg-white border border-slate-200/80 shadow-md hover:-translate-y-1 hover:shadow-2xl transition duration-300">
                  <div className="relative aspect-[4/3] overflow-hidden bg-white p-2 flex items-center justify-center border-b border-slate-100">
                    {c.badge && <span className="absolute left-3 top-3 z-10 rounded-full bg-[#ff6b35] px-3 py-1 text-xs font-black text-white shadow">{c.badge}</span>}
                    {cDiscount > 0 && <span className="absolute right-3 top-3 z-10 rounded-full bg-green-600 px-3 py-1 text-xs font-black text-white shadow">−{formatRub(cDiscount)}</span>}
                  ...
                  <div className="flex flex-1 flex-col p-5"><span className="text-xs font-black uppercase text-[#ff6b35]">{c.brand}</span><h3 className="mt-1 text-lg font-black text-[#1a3a5c] group-hover:text-[#ff6b35] transition">{c.name}</h3><p className="mt-2 text-xs text-slate-500 font-semibold">до {v.area} м² • Шум: {c.noise} • {c.type}</p><div className="mt-auto pt-4 flex items-end justify-between border-t border-slate-100"><div><span className="text-[10px] text-slate-400 font-bold uppercase block">Цена со склада</span><span className="text-xl font-black text-[#1a3a5c]">{formatRub(v.price)}</span></div><span className="px-4 py-2 rounded-full bg-[#1a3a5c] text-white text-xs font-black group-hover:bg-[#ff6b35] transition">Характеристики →</span></div></div>
                </Link>
              );
            })}
          </div>
      ...
      <FAQSection />
      <QuickBookingModal open={bookingOpen} onClose={() => setBookingOpen(false)} serviceName={`Заказ сплит-системы: ${fullName}`} calcDetails={`Выбранная мощность: ${selectedBtu} BTU (до ${variant.area} м²)${isMobile ? "" : isInstallOnRequest ? ", Монтаж: рассчитывается после осмотра объекта" : `, Стандартный монтаж: ${withInstall ? "Да (+18 000 ₽)" : "Нет"}`}, Итог: ${formatRub(totalPrice)}`} />
      ...
      {zoomOpen && allImages.length > 0 && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90" onClick={() => setZoomOpen(false)}>
          ...
          {allImages.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); setActiveImageIdx((activeImageIdx - 1 + allImages.length) % allImages.length); }} className="absolute left-4 flex h-14 w-14 items-center justify-center rounded-full bg-white/10 text-3xl text-white hover:bg-white/20">‹</button>
              <button onClick={(e) => { e.stopPropagation(); setActiveImageIdx((activeImageIdx + 1) % allImages.length); }} className="absolute right-4 flex h-14 w-14 items-center justify-center rounded-full bg-white/10 text-3xl text-white hover:bg-white/20">›</button>
            </>
          )}
          <img src={allImages[activeImageIdx]} alt={`${fullName}`} loading="lazy" decoding="async" className="max-h-[90vh] max-w-[95vw] object-contain" onClick={(e) => e.stopPropagation()} />
          {allImages.length > 1 && <div className="absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full bg-black/60 px-4 py-2 text-sm text-white">{activeImageIdx + 1} / {allImages.length}</div>}
        </div>
      )}
    </div>
  );
